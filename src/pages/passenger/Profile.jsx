  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="card flex items-center gap-4">
        <div className="w-16 h-16 rounded-full bg-primary-100 flex items-center justify-center text-primary-700 text-xl font-heading font-bold">
          {getInitials(user?.name)}
        </div>
        <div>
          <p className="font-semibold text-surface-900">{user?.name}</p>
          <p className="text-sm text-surface-500">{user?.email}</p>
          <span className="badge bg-primary-100 text-primary-700 mt-1">Passenger</span>
        </div>
      </div>
          ...
          <form onSubmit={saveProfile} className="space-y-4">
            <div>
              ...
              <div className="relative">
                ...
                <input name="name" value={form.name} onChange={handle} className="input pl-9" placeholder="Your name" />
              </div>
            </div>
            <div>
            ...
            {['currentPassword', 'newPassword', 'confirmPassword'].map((field, i) => (
              <div key={field}>
                ...
                <div className="relative">
                  ...
                  <input name={field} type="password" required value={pwForm[field]}
                    onChange={handlePw} className="input pl-9" placeholder="••••••••" />
                </div>
              </div>
            ))}
            <button type="submit" disabled={loading} className="btn-primary gap-2">
