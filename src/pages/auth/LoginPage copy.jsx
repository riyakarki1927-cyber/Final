  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-surface-50 px-4">
      <div className="w-full max-w-md">
        ...
        <div className="card mt-4 text-xs text-surface-500 space-y-2">
          <p className="font-medium text-surface-700">Demo accounts (any password works)</p>
          <div className="space-y-1.5">
            {[
              { label: '🛡️ Admin',     email: 'admin@nepride.ai' },
              { label: '👤 Passenger', email: 'riyakarki@gmail.com' },
              { label: '🚗 Driver',    email: 'hari@example.com' },
            ].map(({ label, email }) => (
              <button
                key={email}
                type="button"
                onClick={() => setForm({ email, password: '12345678' })}
                className="w-full flex items-center justify-between px-3 py-2 rounded-lg border border-surface-200 hover:border-primary-300 hover:bg-primary-50 transition-colors text-surface-700"
              >
                <span>{label}</span>
          ...
          <p className="text-surface-400">Tap a row to fill the form, then Sign In.</p>
        </div>
      </div>
    </div>
  );
